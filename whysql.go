...
func GetWhereSql_one(TabName, ParameterStr string, OrderStr string, SortStr string, PageSize, CurrentPage int) (string, error){
	var whereSql string

	return whereSql,nil
}

func GetFirstValue(arr []gorose.Data)string{
	var value string = ""
	if len(arr) > 0 {
	for _, v := range arr[0] {
	value, _ = InterfaceToString(v)
	break
	}
	}
	return value

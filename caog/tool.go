...
const (
	toolDoc = "https://github.com/cao-guang/whysql/blob/master/README.md"
)
...
func toolAction(c *cli.Context) (err error) {
	if c.NArg() == 0 {
		...
		fmt.Println("\n安装工具: caog tool install demo")
		fmt.Println("执行工具: caog tool demo")
		fmt.Println("安装全部工具: caog tool install all")
		fmt.Println("全部升级: caog tool upgrade all")
		fmt.Println("\n详细文档：", toolDoc)
		return
	}

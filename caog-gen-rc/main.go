...
func main()  {
	...
	// Write to file.
	dir := filepath.Dir(".")
	outputName := filepath.Join(dir, "rc.cache.go")
...
func getOptions(opt *options, comment string) {
	...
	if regexp.MustCompile(`\s+//\s*rc:.+`).Match([]byte(comment)) {
		args := strings.Split(common.RegexpReplace(`//\s*rc:(?P<arg>.+)`, comment, "$arg"), " ")
...
func processList(s *common.Source, list *ast.Field) (opt options) {
	...
	opt.ExpireCode = "d.rc" + opt.name + "Expire"
	...
	if len(params) > 2 {
		...
		if len(args) > 1 {
			...
			opt.ExtraArgs = strings.TrimLeft(opt.ExtraArgs,",")
